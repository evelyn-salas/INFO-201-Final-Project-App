...
# Evelyn
v_nv_crime_rate_WA_df <- crime_rate_WA_df
v_nv_crime_rate_WA_df$violent_crime <-
  sapply(
    v_nv_crime_rate_WA_df$MURDER +
      v_nv_crime_rate_WA_df$RAPE +
      v_nv_crime_rate_WA_df$AGASSLT,
    sum
  )


v_nv_crime_rate_WA_df$nonviolent_crime <-
  sapply(
    v_nv_crime_rate_WA_df$ROBBERY +
      v_nv_crime_rate_WA_df$LARCENY +
      v_nv_crime_rate_WA_df$MVTHEFT +
      v_nv_crime_rate_WA_df$ARSON,
    sum
  )

#v_nv_crime_rate_WA_df <- rename(v_nv_crime_rate_WA_df, County = county_name)

joined_summary_data <-
  left_join(
    v_nv_crime_rate_WA_df,
    unemployment_df,
    by = "county_name"
  ) %>%
  select("county_name", "Unemploy_rate", "violent_crime", "nonviolent_crime")


summary_plot_v <-
  ggplot(data = joined_summary_data, mapping = aes(x = Unemploy_rate, y = violent_crime)) +
  geom_point(color = "red", size = 4, shape = 18) + labs(title = "Violent Crime", x = "Unemployment Rate", y = "Crime Rate") +
  geom_smooth(method=lm, se=FALSE, linetype="dashed", color = "black") +
  stat_cor(method= "pearson", label.x = 8, label.y = 3000)


summary_plot_nv <-
  ggplot(data = joined_summary_data, mapping = aes(x = Unemploy_rate, y = nonviolent_crime)) +
  geom_point(color = "blue", size = 4, shape = 18) + labs(title = "Nonviolent Crime", x = "Unemployment Rate", y = "Crime Rate") +
  geom_smooth(method=lm, se=FALSE, linetype="dashed", color = "black") +
  stat_cor(method= "pearson", label.x = 8, label.y = 3000)
...
my_server <- function(input_list, output_list) {
  ...
  output_list$q1_interact_map <- renderLeaflet({
   ...
   if(input_list$stat_type_a == "Crime Rate (per 1000k)"){
     map <- map %>%
  ...
  #summary
  output_list$summary_plot <- renderPlot({
    
    if(input_list$vnv_crime == "Violent Crime"){
      summary_plot <-summary_plot_v
    }
    
    if(input_list$vnv_crime == "Nonviolent Crime"){
      summary_plot <-summary_plot_nv
    }
    
    return(summary_plot)
  })
  
  output_list$summary_analysis <- renderText({
    
    summary <- "Here we can see that despite a general trend in the data, there are far too many outliers to make
a conclusive claim about the relationship between crime rate and unemployment. This relationship, or lack there
of it does not seem contingent on the type of crime either, the correlation coefficients are not even close to
one in either case."
  ...
  output_list$conclusion <- renderText({
    
    conclusion <- "For the purposes of this project, our team decided to explore the relationship between the crime
rate and the unemployment rate in Washington counties. We hypothesized that the two values would be positively
correlated, since we could reason how a higher unemployment rate could motivate people to look for other ways
of making a living. However, our data analysis proved us wrong. There seems to be no correlation between these
two. We also wanted to know if the population of a county had an effect on the crime rate. Once again, our
results where inconclusive. We posit that there must be other factors at play here which are affecting the
dramatic differences in crime rate across Washington counties, but our data shows that population and
unemployment rate aren’t the biggest ones. "
